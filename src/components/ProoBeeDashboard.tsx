                    ...
                    {currentMode === 'overview' ? (
                        ...
                        <div className="h-full flex flex-col items-center gap-6 overflow-y-auto p-4">
                            <div className="bg-white p-8 rounded-[3rem] border-4 border-[#1a1a1a] shadow-[8px_8px_0px_0px_#1a1a1a] text-center max-w-2xl w-full">
                                <div className="w-16 h-16 bg-[#FFD700] border-4 border-[#1a1a1a] rounded-full mx-auto flex items-center justify-center mb-4 shadow-[4px_4px_0px_0px_#1a1a1a]"><Bot size={32} className="text-[#1a1a1a]" /></div>
                                <h2 className="text-3xl font-black italic tracking-tighter mb-1">My Agent Console</h2>
                                <p className="font-bold opacity-40 uppercase tracking-widest text-[10px] mb-6">Manage your trading bee fleet</p>
                                <button onClick={() => setIsCreateModalOpen(true)} className="px-8 py-3 bg-[#1a1a1a] text-white rounded-2xl font-black text-sm shadow-[4px_4px_0px_0px_#FFD700] hover:scale-105 transition-transform active:translate-y-1">HATCH NEW BEE</button>
                            </div>
                                ...
                                {myAgents.map((agent) => (
                                    <div key={agent.id} className={`bg-white p-6 rounded-[2rem] border-4 border-[#1a1a1a] shadow-[4px_4px_0px_0px_#1a1a1a] flex justify-between items-center transition-all ${!agent.is_active ? 'opacity-60 grayscale' : ''}`}>
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h3 className="font-black text-lg italic leading-tight">{agent.name}</h3>
                                                <p className="text-[10px] font-black opacity-40 uppercase tracking-tighter">{agent.provider} • Yield: {agent.yield}%</p>
                                            </div>
                                        </div>
                                        <button
