                ...
                <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#FFD700]">
                    <div className="bg-white p-10 rounded-[3rem] border-4 border-[#1a1a1a] shadow-[8px_8px_0px_0px_#1a1a1a] text-center max-w-md w-full mx-4">
                        ...
                        <form onSubmit={handleAuth} className="space-y-4 text-left">
                            ...
                            <button type="submit" className="w-full bg-[#1a1a1a] text-white py-4 rounded-2xl font-black text-lg shadow-[4px_4px_0px_0px_#1a1a1a]">{isSignUp ? 'Create Account' : 'Connect'}</button>
                        </form>
                        <button onClick={() => setIsSignUp(!isSignUp)} className="mt-6 text-xs font-black uppercase opacity-60 underline">{isSignUp ? 'Log In' : 'Sign Up'}</button>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#1a1a1a]/40 backdrop-blur-sm p-4">
                    <div className="bg-white w-full max-w-lg rounded-[2.5rem] border-4 border-[#1a1a1a] shadow-[8px_8px_0px_0px_#1a1a1a] overflow-hidden">
                        ...
                        <form onSubmit={handleCreateAgent} className="p-8 space-y-5">
                            <input value={newAgentName} onChange={(e) => setNewAgentName(e.target.value)} placeholder="Bee Name" className="w-full px-5 py-3 rounded-2xl border-2 border-[#1a1a1a] font-bold outline-none" required />
                            <div className="grid grid-cols-2 gap-4">
                                <select value={selectedProvider} onChange={(e) => setSelectedProvider(e.target.value as AIProvider)} className="w-full px-4 py-3 rounded-2xl border-2 border-[#1a1a1a] font-bold bg-white"><option value="Google">Google (Gemini)</option><option value="OpenAI">OpenAI</option></select>
                                <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="API Key" className="w-full px-4 py-3 rounded-2xl border-2 border-[#1a1a1a] font-bold" required />
                            </div>
                            <textarea value={newAgentPrompt} onChange={(e) => setNewAgentPrompt(e.target.value)} placeholder="Behavior Prompt" className="w-full h-32 px-5 py-4 rounded-2xl border-2 border-[#1a1a1a] font-bold outline-none resize-none bg-[#fdfcf0]" required />
                            <button type="submit" className="w-full py-4 bg-[#1a1a1a] text-white rounded-2xl font-black text-lg shadow-[4px_4px_0px_0px_#FFD700]">HATCH AGENT</button>
                        </form>
                    </div>
                </div>
            )}
                ...
                {currentMode === 'overview' ? (
                    ...
                    <div className="h-full flex flex-col items-center gap-6 overflow-y-auto p-4">
                        <div className="bg-white p-8 rounded-[3rem] border-4 border-[#1a1a1a] shadow-[8px_8px_0px_0px_#1a1a1a] text-center max-w-2xl w-full">
                            ...
                            <h2 className="text-3xl font-black italic mb-6 uppercase">My Agent Console</h2>
                            <button onClick={() => setIsCreateModalOpen(true)} className="px-8 py-3 bg-[#1a1a1a] text-white rounded-2xl font-black shadow-[4px_4px_0px_0px_#FFD700]">HATCH NEW BEE</button>
                        </div>
